...
from test import run_upscale

#load_dotenv()
TG_TOKEN = os.getenv('TG_TOKEN')
#MODEL_DATA = os.getenv('MODEL_DATA', 'SG161222/Realistic_Vision_V2.0')
MODEL_DATA = os.getenv('MODEL_DATA', 'Lykon/DreamShaper')
#MODEL_DATA = os.getenv('MODEL_DATA', 'nitrosocke/redshift-diffusion')
...
if MODEL_DATA.startswith("stabilityai/"):
  model_revision = "fp16"
else:
  model_revision = None

pipe = StableDiffusionPipeline.from_pretrained(MODEL_DATA, torch_dtype=torch.float16, revision=model_revision, use_auth_token=USE_AUTH_TOKEN)
pipe = pipe.to(device)

img2imgPipe = StableDiffusionImg2ImgPipeline.from_pretrained(MODEL_DATA, torch_dtype=torch.float16, revision=model_revision, use_auth_token=USE_AUTH_TOKEN)
img2imgPipe = img2imgPipe.to("cuda")
pipe.enable_xformers_memory_efficient_attention()
pipe = StableDiffusionPipeline.from_pretrained(MODEL_DATA, torch_dtype=torch.float16, revision=model_revision,)
pipe = pipe.to(device)
pipe.enable_xformers_memory_efficient_attention()
...
def get_try_again_markup():
    keyboard = [[InlineKeyboardButton("Try again", callback_data="TRYAGAIN"), InlineKeyboardButton("Variations", callback_data="VARIATIONS"), InlineKeyboardButton("Upscale", callback_data="UPSCALE")]]
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == "TRYAGAIN":
        ...
        await context.bot.delete_message(chat_id=progress_msg.chat_id, message_id=progress_msg.message_id)
        await context.bot.send_photo(update.effective_user.id, image_to_bytes(im), caption=f'"{prompt}"', reply_markup=get_try_again_markup(), reply_to_message_id=replied_message.message_id)
        ...
        path = str(photo_path)
        new_path = run_upscale(path)
        im = open(new_path, 'rb')
        seed = 0
        prompt = 'Upscale'
        await context.bot.delete_message(chat_id=progress_msg.chat_id, message_id=progress_msg.message_id)
        await context.bot.send_photo(update.effective_user.id, im, caption=f'"{prompt}"', reply_markup=get_try_again_markup(), reply_to_message_id=replied_message.message_id)
